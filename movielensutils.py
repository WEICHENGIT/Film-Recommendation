...
def objective(P, Q0, R, mask, rho):
    ...
    grad_P = np.transpose(Q0).dot((Q0.dot(P)-R)*mask) + rho*P  # todo
...
def total_objective(P, Q, R, mask, rho):
    ...
    grad_P = np.transpose(Q).dot((Q.dot(P)-R)*mask) + rho*P  # todo

    grad_Q = ((Q.dot(P)-R)*mask).dot(np.transpose(P))+ rho*Q  # todo
